...
const MiniCart: React.FC<MiniCartProps> = ({ isOpen, onClose, cart, onUpdateQuantity, onRemove }) => {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-stone-200 bg-emerald-950">
          <div className="flex items-center space-x-2">
            <ShoppingBag className="h-5 w-5 text-gold-400" />
            <h3 className="text-lg font-bold text-white">আপনার ব্যাগ</h3>
            <span className="bg-gold-500 text-emerald-950 text-xs font-bold px-2 py-0.5 rounded-full">
              {cart.reduce((acc, item) => acc + item.quantity, 0)}
            </span>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-emerald-900 rounded-full transition text-emerald-200 hover:text-white"
          >
            <X size={20} />
          </button>
        </div>
          ...
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
              ...
              <Link
                href="/shop"
                onClick={onClose}
                className="bg-emerald-700 text-white px-6 py-2.5 rounded-full font-medium hover:bg-emerald-800 transition"
              >
                কেনাকাটা শুরু করুন
              </Link>
            </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      className="block"
                    >
                      <h4 className="text-sm font-bold text-stone-800 line-clamp-2 mb-1 hover:text-emerald-700 transition">
                        {item.name}
                      </h4>
                    </Link>
                    <p className="text-xs text-emerald-600 font-semibold mb-2">৳ {item.price}</p>
        ...
        {cart.length > 0 && (
          <div className="border-t border-stone-200 p-4 bg-white">
            <div className="space-y-2 mb-4">
              ...
              <div className="flex justify-between text-base font-bold text-emerald-900 pt-2 border-t border-stone-200">
                <span>মোট</span>
                <span>৳ {total}</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Link
                href="/checkout"
                onClick={onClose}
                className="block w-full bg-emerald-700 text-white text-center py-3 rounded-xl font-bold hover:bg-emerald-800 transition shadow-lg"
              >
                চেকআউট করুন
              </Link>
              <Link
