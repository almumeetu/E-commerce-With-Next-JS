  ...
  return (
    <div className="relative z-30 max-w-4xl mx-auto px-4 -mt-10 mb-12">
      <div className="bg-white/10 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-6 md:p-8 flex flex-col md:flex-row items-center justify-between overflow-hidden relative text-white">
        ...
        {/* Glow Effect */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-gradient-to-r from-emerald-500/20 to-gold-500/20 blur-3xl -z-10"></div>

        <div className="mb-6 md:mb-0 text-center md:text-left z-10 flex items-center">
          <div className="bg-gold-400/20 p-3 rounded-full mr-4 hidden md:block">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gold-400"><path d="M12 2v20M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></svg>
          </div>
          <div>
...
const TimeUnit: React.FC<{ value: number, label: string, toBangla: (n: number) => string }> = ({ value, label, toBangla }) => (
  <div className="flex flex-col items-center">
    <div className="bg-emerald-950/60 backdrop-blur-md w-14 h-16 md:w-20 md:h-24 rounded-2xl flex items-center justify-center border border-white/10 shadow-lg mb-2 relative overflow-hidden group">
      <div className="absolute inset-0 bg-gradient-to-b from-white/5 to-transparent opacity-50"></div>
      <span className="text-2xl md:text-4xl font-bold text-gold-400 font-mono relative z-10 group-hover:scale-110 transition-transform">
        {toBangla(value)}
      </span>
    </div>
    <span className="text-xs md:text-sm text-emerald-100/80 font-medium tracking-wide">{label}</span>
  </div>
);
